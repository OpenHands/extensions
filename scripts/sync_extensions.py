...
def parse_frontmatter(text: str) -> dict[str, str | list[str]]:
    ...
    Uses simple regex parsing (no YAML dependency).  Gracefully returns
    partial results when individual fields are malformed.
    """
    m = re.match(r"^---\n(.*?)\n---", text, re.DOTALL)
    if not m:
        return {}
    ...
    for line in block.splitlines():
        ...
        if in_triggers:
            ...
            if stripped.startswith("- "):
                ...
                continue  # skip blanks and comments inside triggers list
            else:
                break
...
def _entry_type(source: str) -> str:
    ...
    if source.startswith("./plugins/") or source.startswith("../plugins/"):
        return "plugin"
    # Fallback: check if the path resolves to an actual directory
