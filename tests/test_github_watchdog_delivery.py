...
def test_watchdog_updates_accepted_branch_for_fresh_review(tmp_path, monkeypatch):
    cls = worker("github-delivery-watchdog", tmp_path, monkeypatch).DeliveryWatchdog
    run = object.__new__(cls)
    run.config = {}
    sha = "a" * 40
    ...
    run.ci_passed = lambda sha: True

    assert run.merge(1, sha) == {"message": "Updating pull request branch."}
    assert calls[-2:] == [
        ("POST", "/issues/1/labels", {"labels": ["openhands-review"]}),
        ("PUT", "/pulls/1/update-branch", {"expected_head_sha": sha}),
    ]
