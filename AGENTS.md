...
- The integration catalog is unified into a single JSON asset, `integrations/integration-catalog.json`, which is the **single source of truth read at runtime by both** the JS (`@openhands/extensions`) and Python (`openhands_extensions` under `python/`) packages. Do NOT add per-language duplicates of provider data. `scripts/build-integration-catalog.mjs` generates the asset (writing BOTH the root copy and `python/openhands_extensions/integration-catalog.json` in one pass); after editing any catalog source, re-run `node scripts/build-integration-catalog.mjs` (alias `npm run build:integration-catalog`) and commit both copies. `tests/test_integration_catalog_in_sync.py` asserts the checked-in JSON == a fresh build, JS accessors == Python accessors, the two packaged copies are byte-identical, and the `mcp`/`oauth` filter accessors select the right entries. JS exposes `listIntegrationCatalog({ mcp, oauth })` / `listOAuthProviderCatalog` / `getOAuthProviderRegistrationDefaults` / `defaultManagedConnectors`; Python mirrors with `list_integration_catalog(mcp=, oauth=)` / `list_oauth_provider_catalog` / `get_oauth_provider_registration_defaults` / `default_managed_connectors`. The legacy `managedConnectorMigration` / `legacyScopeBundles` / `canonicalServerUrl` / `errorHints` mechanism was removed intentionally; providers declare only standard OAuth config as data.
