...
- Marketplace-listed skills and plugin directories are expected to ship `.plugin/plugin.json` plus vendor symlinks (`.claude-plugin`, `.codex-plugin`). If you add a new skill or plugin manifest, rerun `python scripts/sync_extensions.py symlinks` before testing.
