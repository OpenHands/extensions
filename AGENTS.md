...
- `integrations/integration-catalog.json` is the **single hand-authored source of truth** (NOT generated from any `.mjs`/`.js` source), read at runtime by both the JS (`@openhands/extensions`) and Python (`openhands_extensions` under `python/`) packages. It is mirrored byte-for-byte into `python/openhands_extensions/integration-catalog.json`. Do NOT add per-language duplicates of provider data, and do NOT reintroduce a build script that regenerates it. Provider-specific OAuth knowledge is merged into each integration as a minimal `oauthProvider` override object (`null` when the integration has no OAuth provider); only fields whose OAuth-context values differ from the integration's connector view (`description`, `docsUrl`, `popularityRank`) are stored. The `providers[]` view, `supportsMcp`/`supportsOauth` flags, and `defaultManagedConnectors` are all **derived at runtime** by `integrations/index.js` and `python/openhands_extensions/integrations.py` from the canonical asset + `defaultManagedConnectorSlugs`. To edit the catalog: edit `integrations/integration-catalog.json` directly, then copy it to the Python location, and update the matching `integrations/catalog/<id>.json` file (a CI parity test asserts every `catalog/<id>.json` equals its master entry and the two packaged JSON copies are byte-identical). JS exposes `listIntegrationCatalog({ mcp, oauth })` / `listOAuthProviderCatalog` / `getOAuthProviderRegistrationDefaults` / `defaultManagedConnectors`; Python mirrors with `list_integration_catalog(mcp=, oauth=)` / `list_oauth_provider_catalog` / `get_oauth_provider_registration_defaults` / `default_managed_connectors`. The legacy `managedConnectorMigration` / `legacyScopeBundles` / `canonicalServerUrl` / `errorHints` mechanism was removed intentionally; providers declare only standard OAuth config as data.
