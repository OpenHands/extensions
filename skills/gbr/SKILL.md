...
name: gbr
description: >
  Pair a phone running Build Remote Agent to this OpenHands desktop session.
  Requires gbr-agent run on the host. Attach via Bot API 127.0.0.1:8788 or gbr-mcp.
  Use when the user wants mobile spectator / inject into an OpenHands (Canvas, CLI, or SDK) session.
triggers:
  - gbr
  - build remote agent
  - pair phone
...
One adapter for OpenHands. Protocol `gbr/1`. No fourth pair protocol.

Independent product by Linespotting AB. Not affiliated with xAI or SpaceX.

OpenHands owns the agent loop (Canvas / CLI / Software Agent SDK). The phone
spectates and can veto or inject; it does not orchestrate.
...
4. PC: `gbr-agent run` (keep it running beside `agent-canvas` / the agent-server).
...
Phone is spectator + veto, not orchestrator. Do not open extra ports or invent a Canvas-native pair flow.
...
SDK `mcp_config` is a flat `dict[str, MCPServer]` (not a `.mcp.json` wrapper).
...
Never put mailbox keys, `X-GBR-Key`, or `~/.gbr/` in this skill, in `mcp_config`, or in git.
