...
def _build_review_prompt(repo: str, pr: dict, head_sha: str, label_event: dict, repo_review_guide: str | None = None) -> str:
    ...
    return (
        ...
        "3. Inspect the PR discussion, existing review comments, changed files, and the diff, "
        ...
        "4. Ground every finding in the workspace code. Before using an inline location, verify that "
        "the path and line are part of this pull request's diff.\n"
        f"5. Publish one review with `POST /repos/{repo}/pulls/{number}/reviews`, using "
        ...
        "6. If a finding cannot be attached to a changed line, put it in the review body instead. "
        "If the API rejects the inline positions, retry with every finding in the body and no `comments` array.\n"
        "7. Begin the review body with this disclosure: "
        "`_This review was posted by an AI agent (OpenHands)._`\n"
        "8. End the review body with a verdict on its own line: either `✅ APPROVED` "
        ...
        "If publishing still fails after the fallback in step 6, output the complete review text "
