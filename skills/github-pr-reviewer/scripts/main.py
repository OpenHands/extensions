...
# Verdicts a published review body may end with. The deterministic completion
# handler reads the last of these it finds; `MAINTAINER_DECISION REQUIRED` is a
# scope stop, which requests a human maintainer without approving the PR.
...
def _build_review_prompt(
    ...
    *,
    ...
    return (
        ...
        "   Use `gh` or GitHub REST API calls with `{github_token_secret}`; never print secret values.\n"
        "3. SCOPE GATE - run this before inspecting changed files, reading the diff, or running "
        "any test. Using the repository guidance you just read (its scope categories and "
        "ownership boundaries), decide whether this change belongs in this repository and has the "
        "product/architecture direction it needs. Do this from the PR metadata and discussion; do "
        "not audit implementation details yet.\n"
        "   Stop at this gate and publish a maintainer-decision review when any of the following "
        "applies:\n"
        "   - Wrong repository: the change belongs in a repository other than this one.\n"
        "   - Unresolved product/architecture decision: the change needs a product or architecture "
        "decision a maintainer has not made and that is not present in a linked issue or the PR.\n"
        "   - Obsolete/duplicate/current-direction conflict: the change is obsolete, duplicates "
        "existing work, or conflicts with the repository's current direction.\n"
        "   - Another repo-defined scope reason from the repository guidance.\n"
        f"   When you stop, publish exactly one review with `POST /repos/{repo}/pulls/{number}/reviews`, "
        "using `commit_id` equal to the Head SHA above and `event: COMMENT`. Do not run tests, "
        "inspect the diff, or report implementation findings. In the body:\n"
        "   - Name the scope reason, using one of: wrong repository, unresolved "
        "product/architecture decision, obsolete/duplicate or current-direction conflict, or the "
        "repo-defined reason.\n"
        "   - For a wrong repository, name the likely owning repository when the evidence is "
        "sufficient; omit it when it is not.\n"
        "   - End the body with a verdict on its own line: `🛑 MAINTAINER DECISION REQUIRED`.\n"
        "   This outcome is not an approval and does not merge the PR. The deterministic "
        "automation requests a configured maintainer.\n"
        ...
        "9. End a technical review body with a verdict on its own line: either `✅ APPROVED` "
        "or `🔄 CHANGES REQUESTED`. A scope-stopped review instead ends with "
